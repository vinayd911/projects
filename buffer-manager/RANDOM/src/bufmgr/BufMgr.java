...
//-------------------------------------------------------------
    ...
    float pageLoadBHR = 0;
    int[][] pageReferCount =PAGE_LOADING_BASED_MAX_HITS;
    ...
    public BufMgr(int numbufs)
    {
        ...
        replacer = new RandomRP(this);   // change Policy to replacement class name
    }
    ...
    public PageId newPage(Page firstpg, int run_size)
    {
        ...
        try {
            ...
            // Increment uniquePageLoads when a new page is successfully loaded.
            uniquePageLoads++;
        }
        catch (Exception e) {
    ...
    public void freePage(PageId pageno)
    {
        ...
        if(tempfd != null)
        {
            ...
            tempfd.pincnt = 0;
            tempfd.dirty = false;
            tempfd.state = RandomRP.AVAILABLE;
    ...
    public void pinPage(PageId pageno, Page page, boolean skipRead)
    {
        ...
        if(tempfd != null)
        {
            if (tempfd.state == Policy.PINNED) {
                totalPageHits++; // Increment the total page hit count.
            }
            ...
            else
            {
                ...
                // Increment Page Load Hits here
                pageLoadHits++;
                // Increment Page Load Requests here
                pageLoadRequests++;
        ...
        else
        {
            ...
            // Increment pageFaults when a new page is needed from disk.
            pageFaults++;
...
// some BHR code may go here
            ...
            // Update counters for page load requests and hits.
            pageLoadRequests++;
            pageLoadHits++;
...
// some BHR code may go here
    ...
    public void unpinPage(PageId pageno, boolean dirty)
    {
        ...
        else
        {
            ...
            tempfd.dirty = dirty;
            if(tempfd.pincnt== 0)
                tempfd.state = RandomRP.REFERENCED;
    ...
    public void printBhrAndRefCount(){
        ...
        for (int i = 0; i < pageReferCount.length ; i++) {
            if (pageReferCount[i][0] > pageReferCount[i][1]) pageReferCount[i][1] = pageReferCount[i][0];
            pageReferCount[i][0] = 0;
        }
        //Sort and print top k page references here. done by this code
        sortbyColumn(pageReferCount, 1);

        System.out.println("The top k (10) referenced pages are:");
        System.out.println("       Page No.\t\tNo. of references");

        for (int i = 0; i < 10 ; i++)
            System.out.println("\t"+pageReferCount[i][2]+"\t\t"+pageReferCount[i][1]);

        System.out.println("+----------------------------------------+");
		// System.out.println("pageReferCount.length: "+pageReferCount.length);
        // *for (int i = 0; i < pageReferCount.length ; i++)
        // *System.out.println("\t"+pageReferCount[i][2]+"\t\t"+pageReferCount[i][1]+"\t\t"+pageReferCount[i][0]);*/
    }
